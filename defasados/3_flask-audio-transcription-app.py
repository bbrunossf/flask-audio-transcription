...
# ─── ADIÇÃO 2: duas novas rotas ───
@app.route('/api/transcripts', methods=['GET'])
def list_transcripts():
    """Return a list of saved transcript files with metadata."""
    transcripts_dir = app.config['TRANSCRIPTS_FOLDER']
...
def get_transcript_content(filename):
    """Return the full content of a saved transcript."""
    safe_name = secure_filename(filename)
    filepath = os.path.join(app.config['TRANSCRIPTS_FOLDER'], safe_name)
